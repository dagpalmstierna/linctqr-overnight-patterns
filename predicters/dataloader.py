...
class DataLoader():
    ...
    def get_etf_data(self, src="SYBJ.DE"):
        ...
        reversal = ((df["Close"] < next_open) & (df["Close"] < df["Open"])) #| ((df["Close"] > next_open) & (df["Close"] > df["Open"])) 
    ...
    def balance_training_data_with_time(self, train_df, window="6M"):
        balanced_dfs = []
        # Dela upp i grupper baserat på tidsfönster med pd.Grouper
        for group_time, group in train_df.groupby(pd.Grouper(freq=window)):
            ...
            # Anpassa k_neighbors så att vi inte ber om fler grannar än vad som finns
                ...
                try:
                    ...
                    print(f"SMOTE misslyckades för grupp {group_time}: {e}")
                    group_balanced = group.copy()
                
            balanced_dfs.append(group_balanced)

        # Sortera och återmontera alla grupper
    ...
    def apply_pca(self, n_components=0.95):
        ...
        # Utför PCA med det valda antalet komponenter
        ...
        # Transform "Reversal" column to 1 for True and 0 for False
    ...
    def get_data(self):
        return self.df
# dl = DataLoader()
# print(len(dl.get_data()))
# print(dl.df["Reversal"].value_counts())
